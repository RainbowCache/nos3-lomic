...
/*
** File: coveragetest_cannon_app.c
**
** Purpose:
** Coverage Unit Test cases for the CANNON Application
**
** Notes:
** This implements various test cases to exercise all code
** paths through all functions defined in the CANNON application.
**
...
#include "cannon_app_coveragetest_common.h"
#include "ut_cannon_app.h"

/* to get the CANNON_LIB_Function() declaration */
...
void Test_CANNON_AppMain(void)
{
    ...
    /*
     * CANNON_AppMain does not return a value,
     ...
     * The first is for CANNON_AppInit().  As this is in the same
     ...
     * the CANNON_AppInit() failure path was taken.
     */
    CANNON_AppMain();
     ...
     */
    UtAssert_True(CANNON_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                  "CANNON_AppData.RunStatus (%lu) == CFE_ES_RunStatus_APP_ERROR",
                  (unsigned long)CANNON_AppData.RunStatus);
    ...
    UT_CheckEvent_Setup(&EventTest, CANNON_PIPE_ERR_EID, "CANNON: SB Pipe Read Error = %d");
     ...
     */
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_PIPE_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CANNON_AppInit(void)
{
    /*
     * Test Case For:
    ...
    UT_TEST_FUNCTION_RC(CANNON_AppInit(), CFE_SUCCESS);
    ...
    UT_SetDeferredRetcode(UT_KEY(CFE_EVS_Register), 1, CFE_EVS_INVALID_PARAMETER);
    UT_TEST_FUNCTION_RC(CANNON_AppInit(), CFE_EVS_INVALID_PARAMETER);
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_ES_WriteToSysLog)) == 1, "CFE_ES_WriteToSysLog() called");

    UT_SetDeferredRetcode(UT_KEY(CFE_SB_CreatePipe), 1, CFE_SB_BAD_ARGUMENT);
    UT_TEST_FUNCTION_RC(CANNON_AppInit(), CFE_SB_BAD_ARGUMENT);
...
void Test_CANNON_ProcessCommandPacket(void)
{
    /*
     * Test Case For:
     * void CANNON_ProcessCommandPacket
     */
    /* a buffer large enough for any command message */
    union
    {
        CFE_SB_Buffer_t      SBBuf;
        CANNON_NoArgs_cmd_t  Noop;
    } TestMsg;
    ...
    UT_CheckEvent_Setup(&EventTest, CANNON_PROCESS_CMD_ERR_EID, NULL);

    /*
     * The CFE_MSG_GetMsgId() stub uses a data buffer to hold the
     * message ID values to return.
     */
    TestMsgId = CFE_SB_ValueToMsgId(CANNON_CMD_MID);
    FcnCode   = CANNON_NOOP_CC;
    ...
    UtAssert_True(EventTest.MatchCount == 0, "CANNON_CMD_ERR_EID not generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    CANNON_ProcessCommandPacket();
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CANNON_ProcessGroundCommand(void)
{
    /*
     * Test Case For:
     * void CANNON_ProcessGroundCommand
     */
    CFE_SB_MsgId_t    TestMsgId = CFE_SB_ValueToMsgId(CANNON_CMD_MID);
    CFE_MSG_FcnCode_t FcnCode;
    size_t            Size;
    ...
    UT_CheckEvent_Setup(&EventTest, CANNON_CMD_NOOP_INF_EID, NULL);
    CANNON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_CMD_NOOP_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);

    /* test dispatch of RESET */
    FcnCode = CANNON_RESET_COUNTERS_CC;
    Size    = sizeof(TestMsg.Reset);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &Size, sizeof(Size), false);
    UT_CheckEvent_Setup(&EventTest, CANNON_CMD_RESET_INF_EID, NULL);
    CANNON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_CMD_RESET_INF_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &TestMsgId, sizeof(TestMsgId), false);
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &FcnCode, sizeof(FcnCode), false);
    UT_CheckEvent_Setup(&EventTest, CANNON_CMD_ERR_EID, NULL);
    CANNON_ProcessGroundCommand();
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_CMD_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}

void Test_CANNON_ReportHousekeeping(void)
{
    /*
     * Test Case For:
     * void CANNON_ReportHousekeeping()
     */
    CFE_MSG_Message_t *MsgSend;
    CFE_MSG_Message_t *MsgTimestamp;
    CFE_SB_MsgId_t     MsgId = CFE_SB_ValueToMsgId(CANNON_REQ_HK_TLM);

    /* Set message id to return so CANNON_Housekeeping will be called */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetMsgId), &MsgId, sizeof(MsgId), false);
    ...
    UtAssert_True(MsgSend == &CANNON_AppData.HkTelemetryPkt.TlmHeader.Msg, "CFE_SB_TransmitMsg() address matches expected");

    /* Confirm timestamp msg address */
    UtAssert_True(UT_GetStubCount(UT_KEY(CFE_SB_TimeStampMsg)) == 1, "CFE_SB_TimeStampMsg() called once");
    UtAssert_True(MsgTimestamp == &CANNON_AppData.HkTelemetryPkt.TlmHeader.Msg,
                  "CFE_SB_TimeStampMsg() address matches expected");
}

void Test_CANNON_VerifyCmdLength(void)
{
    /*
     * Test Case For:
     * bool CANNON_VerifyCmdLength
     */
    UT_CheckEvent_t   EventTest;
    size_t            size    = 1;
    CFE_MSG_FcnCode_t fcncode = 2;
    CFE_SB_MsgId_t    msgid   = CFE_SB_ValueToMsgId(CANNON_CMD_MID);

    /*
     * test a match case
     */
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetSize), &size, sizeof(size), false);
    UT_CheckEvent_Setup(&EventTest, CANNON_LEN_ERR_EID, NULL);

    CANNON_VerifyCmdLength(NULL, size);

    /*
     * Confirm that the event was NOT generated
     */
    UtAssert_True(EventTest.MatchCount == 0, "CANNON_LEN_ERR_EID NOT generated (%u)",
                  (unsigned int)EventTest.MatchCount);
    ...
    UT_SetDataBuffer(UT_KEY(CFE_MSG_GetFcnCode), &fcncode, sizeof(fcncode), false);
    UT_CheckEvent_Setup(&EventTest, CANNON_LEN_ERR_EID, NULL);
    CANNON_VerifyCmdLength(NULL, size + 1);

    /*
     * Confirm that the event WAS generated
     */
    UtAssert_True(EventTest.MatchCount == 1, "CANNON_LEN_ERR_EID generated (%u)",
                  (unsigned int)EventTest.MatchCount);
}
...
{
    ADD_TEST(CANNON_AppMain);
    ADD_TEST(CANNON_AppInit);
    ADD_TEST(CANNON_ProcessCommandPacket);
    ADD_TEST(CANNON_ProcessGroundCommand);
    ADD_TEST(CANNON_ReportHousekeeping);
    ADD_TEST(CANNON_VerifyCmdLength);
}
