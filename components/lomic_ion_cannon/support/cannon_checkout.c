...
** Include Files
*/
#include "cannon_checkout.h"


/*
** Global Variables
*/
uart_info_t CANNONUart;
CANNON_Device_HK_tlm_t CANNONHK;
CANNON_Device_Data_tlm_t CANNONData;
...
int process_command(int cc, int num_tokens, char tokens[MAX_INPUT_TOKENS][MAX_INPUT_TOKEN_SIZE])
{
    ...
    {	
        ...
        case CMD_NOOP:
            ...
            {
                status = CANNON_CommandDevice(&CANNONUart, CANNON_DEVICE_NOOP_CMD, 0);
        ...
        case CMD_HK:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = CANNON_RequestHK(&CANNONUart, &CANNONHK);
                if (status == OS_SUCCESS)
                {
                    OS_printf("CANNON_RequestHK command success\n");
                }
                else
                {
                    OS_printf("CANNON_RequestHK command failed!\n");
                }
            }
            break;
        ...
        case CMD_CANNON:
            if (check_number_arguments(num_tokens, 0) == OS_SUCCESS)
            {
                status = CANNON_RequestData(&CANNONUart, &CANNONData);
                if (status == OS_SUCCESS)
                {
                    OS_printf("CANNON_RequestData command success\n");
                }
                else
                {
                    OS_printf("CANNON_RequestData command failed!\n");
                }
            }
            break;
        ...
        case CMD_CFG:
            if (check_number_arguments(num_tokens, 1) == OS_SUCCESS)
            {
                ...
                status = CANNON_CommandDevice(&CANNONUart, CANNON_DEVICE_CFG_CMD, config);
                if (status == OS_SUCCESS)
                {
...
int main(int argc, char *argv[]) 
{
    ...
    CANNONUart.deviceString = CANNON_CFG_STRING;
    CANNONUart.handle = CANNON_CFG_HANDLE;
    CANNONUart.isOpen = PORT_CLOSED;
    CANNONUart.baud = CANNON_CFG_BAUDRATE_HZ;
    status = uart_init_port(&CANNONUart);
    if (status == OS_SUCCESS)
    {
        printf("UART device %s configured with baudrate %d \n", CANNONUart.deviceString, CANNONUart.baud);
    }
    else
    {
        printf("UART device %s failed to initialize! \n", CANNONUart.deviceString);
        run_status = OS_ERROR;
    }
    ...
    // Close the device 
    uart_close_port(&CANNONUart);

    OS_printf("Cleanly exiting cannon application...\n\n"); 
    return 1;
}
