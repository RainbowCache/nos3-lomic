...
do
    ...
    cp -rf $BASE_DIR/cfg/models/* $USER_NOS3_DIR/42/Model/
    xhost +local:*
    ...
    gnome-terminal --tab --title=$SC_NUM" - Thruster Sim" -- $DFLAGS -v $SIM_DIR:$SIM_DIR --name $SC_NUM"_thruster_sim" --network=$SC_NETNAME -w $SIM_BIN $DBOX ./nos3-single-simulator $SC_CFG_FILE lomic_thruster_sim 
    gnome-terminal --tab --title=$SC_NUM" - Ion Cannon Sim" -- $DFLAGS -v $SIM_DIR:$SIM_DIR --name $SC_NUM"_cannon_sim"   --network=$SC_NETNAME -w $SIM_BIN $DBOX ./nos3-single-simulator $SC_CFG_FILE lomic_cannon_sim 
    echo ""
done
